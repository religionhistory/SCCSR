...
LEFT JOIN
    (SELECT id, name_id, expert_source_id, created_by_id, description AS entry_description, data_source_id FROM public.polls_entity) en1
ON (b1.entry_id = en1.id)
LEFT JOIN
    (SELECT id AS created_by_id, CONCAT(first_name, ' ', last_name) AS created_by_name FROM public.auth_user) u1
ON (en1.created_by_id = u1.created_by_id)
LEFT JOIN
...
LEFT JOIN
    (SELECT id AS expert_source_id, user_id FROM public.accounts_expert) e3
ON (en1.expert_source_id = e3.expert_source_id) 
LEFT JOIN
    (SELECT id AS user_id, CONCAT(first_name, ' ', last_name) AS expert_source_name FROM public.auth_user) e4
ON (e3.user_id = e4.user_id) 
LEFT JOIN
    (SELECT id AS editor_id, user_id FROM public.accounts_regionaleditor) re
ON (e1.editor_id = re.editor_id)
LEFT JOIN
    (SELECT id as user_id, CONCAT(first_name, ' ', last_name) as regionaleditor_name FROM public.auth_user) e5
ON (re.user_id = e5.user_id)
LEFT JOIN
    (SELECT id AS editor_id, user_id FROM public.accounts_managingeditor) me
ON (e1.editor_id = me.editor_id)
LEFT JOIN
    (SELECT id as user_id, CONCAT(first_name, ' ', last_name) as managingeditor_name FROM public.auth_user) e6
ON (me.user_id = e6.user_id)
left join 
